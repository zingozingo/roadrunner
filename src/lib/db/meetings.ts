...
/**
 * Create a meeting record from fallback plain-text detection (body_parsed).
 * Similar to createMeetingFromICS but:
 *   - Uses source: "body_parsed"
 *   - Dedup by message_id (one fallback meeting per message)
 *   - No sequence-based update logic (no real ICS UID to track)
 * Never throws — logs errors and returns null.
 */
export async function createMeetingFromFallback(
  parsed: ParsedMeeting,
  messageId: string
): Promise<string | null> {
  try {
    ...
    // Dedup: one fallback meeting per message
    const { data: existing } = await db
      .from("meetings")
      .select("id")
    ...
    if (existing && existing.length > 0) {
      console.log(`[MEETING-FALLBACK] Deduped — meeting already exists for message ${messageId}`);
    ...
    if (error) {
      console.error(`Failed to create fallback meeting: ${error.message}`);
      return null;
    }

    console.log(`[MEETING-FALLBACK] Created meeting: "${parsed.title}" (${parsed.meeting_date})`);
    ...
    try {
      ...
      console.error(`Airtable push failed for fallback meeting ${data.id}:`, err);
    }

    return data.id;
  } catch (err) {
    console.error("createMeetingFromFallback error:", err instanceof Error ? err.message : err);
    return null;
  }
}
