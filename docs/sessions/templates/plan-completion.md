...
1. Detect current state:
   ...
   - Read CLAUDE.md for current stats in header banner
...
4. Update CLAUDE.md:
   - Header banner stats if any changed
   - Directory structure tree if files were added/deleted
   - Decision count
   - Architecture documentation if the plan established new rules (e.g., "all queries in db layer", "routes must be thin wrappers")
   - Read actual current values before updating — don't assume
