...
**Branch:** plan-8/engagement-message-management (8 commits)

### What was accomplished

All 6 tasks across 2 phases completed. Phase 1 (Backend): reassignMessages() service function with full entity cascade (messages → meetings via message_id → notes → tasks), dual re-synthesis (source Option C + target incremental), and POST /api/engagements/[id]/reassign API route supporting 4 actions (move_to_existing, move_to_new, return_to_inbox, discard). Phase 2 (UI): ManageEngagement modal with selectable item list, shared EngagementPicker and CreateEngagementForm extracted from InboxClient, and full integration with engagement detail page.

Post-plan polish added: pending_review reset on inbox return, "New Engagement" label clarity, default title cleanup, discard action (4th action in management modal), Pick Partner repositioning from badge to action button, inbox header count fix.
...
| Components | 36 | 39 |
| Routes | 35 | 36 |
| DB functions | 160 | 173 |
| Decisions | #442 | #447 |

### New files
- `src/lib/engagement-manager.ts` — reassignMessages() + discardFromEngagement()
- `src/app/api/engagements/[id]/reassign/route.ts` — 4-action API route
- `src/components/engagements/ManageEngagement.tsx` — management modal
- `src/components/shared/EngagementPicker.tsx` — shared engagement picker
- `src/components/shared/CreateEngagementForm.tsx` — shared create form

### Decisions logged
#443-#447 (AI naming fix, engagement message management, shared picker/form, discard action, inbox count)
